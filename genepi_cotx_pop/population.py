...
class Population:
    ...
    def transmit_infections(self,transmissions):
        ...
        for idx,transmission in zip(idxs,transmissions):
            ...
            if idx<len(self.infecteds): #superinfection event
                if len(genomes) == 1:
                    type='i_super'
                else:
                    type = 'cotx_super'
                ...
                self.notify_transmission(transmission,i.infection, type)
            else: #infect a naive infection
                ...
                if infection:
                    self.notify_transmission(transmission,infection,type)

    def notify_transmission(self,transmission,infection=None,type=None):
    ...
    def update(self,dt):
        ...
        log.info('%s vectorial capacity=%0.4f',self,V)
